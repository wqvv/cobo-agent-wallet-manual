...
description: |
  Cobo Agentic Wallet skill for the **dev** environment (`--env dev`, API: api-agent-wallet-core.dev.cobo.com).
...
6. **On timeout or failure — do NOT retry blindly.** Follow this decision tree:
   ...
   ├─ One or both missing/empty? → Re-run ONCE: ./scripts/bootstrap-env.sh --env dev --force-download
...
- **Timeout**: Use `timeout 300s ./scripts/bootstrap-env.sh --env dev` or set your exec timeout to 300000ms. The extra margin prevents premature kills on slow cloud networks.
...
```bash
export AGENT_WALLET_API_URL=https://api-agent-wallet-core.dev.cobo.com
```
